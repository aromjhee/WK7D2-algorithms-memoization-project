...
//     // if we do amount % coin === one of the coins that means ans === 0
//     // if ((amount % coin[i]).includes(coins)) 
//     // if (ans === 0 && ans > 0) that's when its success;
//     // every time we subtract we ++
//     // compare the count and return the minimum count
//     // we can use reduce of array of counts to return the minimum
...
//         let numOfOp = Math.floor(amount / coins[i])
//         if (coins.includes(remainder)) {
//             countArr.push(numOfOp + 1);
//         }
//         else if (remainder === 0) countArr.push(numOfOp);
//     }

//     if (countArr.length > 0) {
//         const res = countArr.reduce( (accum, curr) => {
//             if (accum > curr) return curr;
//         });
//         return res;
//     }
